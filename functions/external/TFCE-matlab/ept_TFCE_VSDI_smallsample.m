...
% TAMARA: adapted to small sample sizes, so it performs the permutations on
% all possible combinations

 function Results = ept_TFCE_smallsampla(Data, type, ElecFile, varargin)
...
nPerm = nchoosek(nA+nB, nA); %total n° of possible permutations
permutations= nchoosek(1:nA+nB, nA); %all possible combinations for a sampla of size of A
...
display('Calculating Permutations...')
    ...
    for i   = 1:nPerm
        ...
        % Look into parfor for parallel computing
        permi = permutations(i,:); % we take one row of all possible combinations
        permi = [permi, setdiff(1:nA+nB, permi)]; % we add the rest for the other condition
