function [coord0] = draw_brainref(back0,ref_labels0)
...
j = 1;
for ii = 1:nroi
    title(['draw reference and Press any key to continue' ])
    display(strcat('draw ROI:',ref_labels0{ii}, '-and Press any key to continue. Once you have drawn it, before pressing enter, you can adjust the points by simply dragging '))
    % drawnroi = drawpolygon('LineWidth',1.5,'Color',roicolor(ii,:), 'Label', roi_labels0{ii});
    drawnref = drawpolyline('LineWidth',1.5,'Color',roicolor(ii,:));
    
    pause
    coord0{ii,1} = drawnref.Position;
    
    if j> length(roicolors)
        j = 1;
    else
        j = j+1;
    end
