function [] = tilesrow_nframes_nwaves(movieset, tileset, waveset)
...
%       'waveset': structure with settings for waves in fields:
%           waveset.coord;
%           waveset.r;
...
if isfield(movieset, 'backgr')
    ...
    backgr = movieset.data(:,:,end);
end


% n� tiles
if isfield(tileset, 'ntiles')
ntiles = tileset.ntiles;
else 
    ntiles = 5; 
end
...
ax(ncols) = subplot(nrows, ncols, ncols);
hold on
