...
if isfield(tileset, 'interp')
    
    if tileset.interp ~=0
        factor_interp =tileset.interp;
        flag_smooth=1;
    elseif tileset.interp ==0
        factor_interp =0; %do not interpolate
        flag_smooth= 0;
    end
...
method_interp = 'cubic'; %in all cases
...
for  i = 1:n-1
    ...
    im = imtiles(:,:,i);
    back = background;
    imalph = double(imalpha(:,:,i));
    
    % INTERPOLATION BLOCK ------------------------------------------
    % Smooth before interpolation (if an interpolation value exists)
    if flag_smooth
        im = imgaussfilt(im, 0.7);
        imalph = imgaussfilt(imalph, 0.7);
    end
    % Interpolate (if an interpolation value exists)
    im = interp2(im, factor_interp,'nearest');
    back = interp2(back,factor_interp, method_interp);
    imalph =interp2(imalph, factor_interp, 'nearest');
    % --------------------------------------------------------------
    
    % Plot with the id of the function
    plot_framesoverlaid(im,back, imalph ,0, ax(i), tileset.clims, 0, tileset.thresh, custom_map);
...
plot_framesoverlaid(im,back, imalph ,0, ax(i), tileset.clims, 1, tileset.thresh, custom_map);
tit= strcat(num2str(datatime(timeindx(i))), 'ms');
title(tit, 'fontsize', 6)
set(gca,'XColor', 'none','YColor','none')
...
% Updated 12/01/22 - add tileset.interp (interpolation of images option)
